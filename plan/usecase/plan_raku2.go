...
// planRaku2Usecase らく通プラン関連usecase
type planRaku2Usecase struct {
...
// NewPlanRaku2Usecase インスタンス生成
func NewPlanRaku2Usecase(db *gorm.DB) plan.IPlanUsecase {
	return &planRaku2Usecase{
...
// FetchList プラン一覧取得
func (p *planRaku2Usecase) FetchList(request *plan.ListInput) ([]plan.ListOutput, error) {
...
// Detail プラン詳細
func (p *planRaku2Usecase) Detail(request *plan.DetailInput) (*plan.DetailOutput, error) {
	response := &plan.DetailOutput{}

	if p.PRaku2Repository.MatchesPlanIDAndPropertyID(request.PlanID, request.PropertyID) == false {
		return response, fmt.Errorf("Error: %s", "この施設ではこのプランを閲覧できません。")
	}

	planCh := make(chan plan.HtTmPlanRaku2s)
	childRatesCh := make(chan []plan.HtTmChildRateRaku2s)
	planImageCh := make(chan []image.PlanImagesOutput)
	...
	go p.fetchPlanImages(planImageCh, []plan.HtTmPlanRaku2s{{PlanTable: plan.PlanTable{PlanID: request.PlanID}}})
	...
	roomData, rErr := p.RRaku2Repository.FetchRoomByRoomTypeID(planDetail.RoomTypeID)
	if rErr != nil {
		return response, rErr
	}

	activePlanTables, err := p.PRaku2Repository.FetchActiveByPlanGroupID(planDetail.PlanGroupID)
	if err != nil {
		return response, err
	}
...
func (p *planRaku2Usecase) createPlan(request *plan.SaveInput, tx *gorm.DB) error {
	// plan_codeのチェック
	...
	duplicate := p.PRaku2Repository.CheckPlanCode(request.PropertyID, planCodeList)
	if duplicate > 0 {
	...
	planTables := []plan.HtTmPlanRaku2s{}
	for _, roomTypeID := range request.SelectedRooms {
		...
		planTables = append(planTables, plan.HtTmPlanRaku2s{
			PlanTable: tempPlan,
		})
	}
	if request.PlanGroupID == 0  {
		// プランの新規作成
		if err := planTxRepo.CreatePlansRaku2(planTables); err != nil {
			return err
		}
	} else {
		// 既存のプラングループへのプラン追加
		if err := planTxRepo.MakePlansRaku2(planTables); err != nil {
			return err
		}
	}

	// 子供料金設定の登録
	childRateTables := []plan.HtTmChildRateRaku2s{}
	for _, child := range request.ChildRates {
		for _, createdPlan := range planTables {
			...
			childRateTables = append(childRateTables, plan.HtTmChildRateRaku2s{ChildRateTable: price.ChildRateTable{
	...
	if err := planTxRepo.CreateChildRateRaku2(childRateTables); err != nil {
		return err
	}
	...
	for _, imageData := range request.Images {
		record := []image.HtTmPlanOwnImagesRaku2s{}
		for _, createdPlan := range planTables {
			record = append(record, image.HtTmPlanOwnImagesRaku2s{
				RoomImageRaku2ID: imageData.ImageID,
				PlanID:           createdPlan.PlanID,
				Order:            imageData.Order,
			})
		}
		if err := imageTxRepo.CreatePlanOwnImagesRaku2(record); err != nil {
			return err
		}
	}
	...
	if request.PlanCancelPolicyId != nil {
		for _, createdPlan := range planTables {
			if err := cpTxRepo.UpsertPlanCancelPolicyRelation(utils.WholesalerIDRaku2, createdPlan.PropertyID, createdPlan.PlanID, *request.PlanCancelPolicyId); err != nil {
				return err
			}
		}
	}
	...
	for _, createdPlan := range planTables {
		info := plan.CheckInOutInfo{
			WholesalerID: utils.WholesalerIDRaku2,
...
// Create プラン作成
func (p *planRaku2Usecase) Create(request *plan.SaveInput) error {
	// トランザクション生成
	tx, txErr := p.PRaku2Repository.TxStart()
	if txErr != nil {
		return txErr
	}
	...
	if err := p.createPlan(request, tx); err != nil {
		p.PRaku2Repository.TxRollback(tx)
		return err
	}

	// コミットとロールバック
	if err := p.PRaku2Repository.TxCommit(tx); err != nil {
		p.PRaku2Repository.TxRollback(tx)
		return err
	}
	return nil
...
// Update プラン更新
func (p *planRaku2Usecase) Update(request *plan.SaveInput) error {
	// トランザクション生成
	tx, txErr := p.PRaku2Repository.TxStart()
	if txErr != nil {
		return txErr
	...
	planTxRepo := pInfra.NewPlanRaku2Repository(tx)
	priceTxRepo := priceInfra.NewPriceRaku2Repository(tx)
	...
	if len(updatePlanIDs) > 0 {
		planTable := &plan.HtTmPlanRaku2s{PlanTable: request.PlanTable}
		planTable.IsDelete = false
		if err := planTxRepo.UpdatePlanRaku2(planTable, updatePlanIDs); err != nil {
			p.PRaku2Repository.TxRollback(tx)
			return err
		}
		...
		for _, updatePlanID := range updatePlanIDs {
			for _, child := range request.ChildRates {
				childRateTable := &plan.HtTmChildRateRaku2s{ChildRateTable: price.ChildRateTable{
					ChildRateType: child.ChildRateType,
					PlanID:        updatePlanID,
					Receive:       child.Receive,
					RateCategory:  child.RateCategory,
				...
				if err := planTxRepo.UpdateChildRateRaku2(childRateTable); err != nil {
					p.PRaku2Repository.TxRollback(tx)
					return err
				}
			}
		...
		for _, updatePlanID := range updatePlanIDs {
			...
			if len(prices) > 0 {
				var inputData []price.HtTmPriceRaku2s
				for _, priceData := range prices {
					...
					inputData = append(inputData, price.HtTmPriceRaku2s{
		...
		for _, updatePlanID := range updatePlanIDs {
			// 紐付けする場合のみrequest.PlanCancelPolicyIdに値が入る。紐付けない場合はnilが入る。
			if request.PlanCancelPolicyId != nil {
				if err := cpTxRepo.UpsertPlanCancelPolicyRelation(utils.WholesalerIDRaku2, planTable.PropertyID, updatePlanID, *request.PlanCancelPolicyId); err != nil {
					p.PRaku2Repository.TxRollback(tx)
					return err
				}
			} else {
				if err := cpTxRepo.DeletePlanCancelPolicyRelation(utils.WholesalerIDRaku2, updatePlanID); err != nil {
					p.PRaku2Repository.TxRollback(tx)
					return err
				}
			}
	...
	if len(deletePlanIDs) > 0 {
		if err := planTxRepo.DeletePlanRaku2(deletePlanIDs); err != nil {
			p.PRaku2Repository.TxRollback(tx)
			return err
		}
	}
...
// Delete プラン削除
func (p *planRaku2Usecase) Delete(planID int64) error {
	// トランザクション生成
	tx, txErr := p.PRaku2Repository.TxStart()
	if txErr != nil {
		return txErr
	...
	cpTxRepo := cpInfra.NewCommonCancelPolicyRepository(tx)
	if err := cpTxRepo.DeletePlanCancelPolicyRelation(utils.WholesalerIDRaku2, planID); err != nil {
		p.PRaku2Repository.TxRollback(tx)
		return err
	}

	cioTxRepo := pInfra.NewPlanCommonRepository(tx)
	if err := cioTxRepo.DeleteCheckInOut(utils.WholesalerIDRaku2, planID); err != nil {
		p.PRaku2Repository.TxRollback(tx)
		return err
	}
...
func (p *planRaku2Usecase) UpdateStopSales(request *plan.StopSalesInput) error {
...
func (p *planRaku2Usecase) fetchRooms(ch chan<- []room.HtTmRoomTypeRaku2s, propertyID int64) {
	rooms, roomErr := p.RRaku2Repository.FetchRoomsByPropertyID(room.ListInput{PropertyID: propertyID})
	if roomErr != nil {
...
func (p *planRaku2Usecase) fetchPlans(ch chan<- []plan.HtTmPlanRaku2s, propertyID int64) {
	plans, planErr := p.PRaku2Repository.FetchAllByPropertyID(plan.ListInput{PropertyID: propertyID})
	if planErr != nil {
...
func (p *planRaku2Usecase) fetchRoomImages(ch chan<- []image.RoomImagesOutput, rooms []room.HtTmRoomTypeRaku2s) {
	var roomIDList []int64
	for _, roomData := range rooms {
		roomIDList = append(roomIDList, roomData.RoomTypeID)
	}
	images, imageErr := p.IRaku2Repository.FetchImagesByRoomTypeID(roomIDList)
	if imageErr != nil {
...
func (p *planRaku2Usecase) fetchPlanImages(ch chan<- []image.PlanImagesOutput, plans []plan.HtTmPlanRaku2s) {
	var planIDList []int64
	for _, planData := range plans {
		planIDList = append(planIDList, planData.PlanID)
	}
	images, imageErr := p.IRaku2Repository.FetchImagesByPlanID(planIDList)
	if imageErr != nil {
...
func (p *planRaku2Usecase) fetchPlan(ch chan<- plan.HtTmPlanRaku2s, planID int64) {
	planData, planErr := p.PRaku2Repository.FetchOne(planID)
	if planErr != nil {
		ch <- plan.HtTmPlanRaku2s{}
	}
	ch <- planData
...
func (p *planRaku2Usecase) fetchChildRates(ch chan<- []plan.HtTmChildRateRaku2s, planID int64) {
	childRates, planErr := p.PRaku2Repository.FetchChildRates(planID)
	if planErr != nil {
		ch <- []plan.HtTmChildRateRaku2s{}
	}
	ch <- childRates
}

func (p *planRaku2Usecase) fetchPlanCancelPolicy(ch chan<- *cancelPolicy.HtThPlanCancelPolicyRelations, propertyID int64, planID int64) {
...
func (p *planRaku2Usecase) fetchCheckInOut(ch chan<- *plan.HtTmPlanCheckInOuts, propertyID int64, planID int64) {
...
func (p *planRaku2Usecase) calculateAgeFromChildRateType(childRateType int8) (int8, int8) {
...
func (p *planRaku2Usecase) calcChildRate(rateCategory int8, price int, rate int) int {
	switch rateCategory {
	case 0: // 率
...
func (p *planRaku2Usecase) settingChildPrices(childRates []price.HtTmChildRateRaku2s, planData plan.HtTmPlanRaku2s, priceData price.Price) []int {
	childPrice1 := 0
	childPrice1InTax := 0
	childPrice2 := 0
	childPrice2InTax := 0
