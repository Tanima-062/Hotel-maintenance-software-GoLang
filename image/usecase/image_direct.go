...
// imageDirectUsecase 直仕入れ画像関連usecase
type imageDirectUsecase struct {
	ARepository       account.IAccountRepository
	IDirectRepository image.IImageDirectRepository
	ImageStorage      image.IImageStorage
}

// NewImageDirectUsecase インスタンス生成
func NewImageDirectUsecase(db *gorm.DB) image.IImageUsecase {
	return &imageDirectUsecase{
...
// FetchAll 画像一覧
func (i *imageDirectUsecase) FetchAll(request *image.ListInput) ([]image.ImagesOutput, error) {
	response := []image.ImagesOutput{}

	directImages, err := i.IDirectRepository.FetchImagesByPropertyID(request.PropertyID)
	if err != nil {
		return response, nil
	}
	for _, v := range directImages {
		response = append(response, image.ImagesOutput{
			ImageID:    v.RoomImageDirectID,
...
// Update 画像情報更新
func (i *imageDirectUsecase) Update(request *image.UpdateInput) error {
	// 更新前の対象データ取得
	imageData, err := i.IDirectRepository.FetchImageByRoomImageDirectID(request.ImageID)
	if err != nil {
		return err
	}

	// トランザクション生成
	tx, txErr := i.IDirectRepository.TxStart()
	if txErr != nil {
		return txErr
	}
	...
	// 更新
	if err := imageTxRepo.UpdateImageDirect(request); err != nil {
		i.IDirectRepository.TxRollback(tx)
		return err
	}
	// メイン画像から外す
		...
		for index, mainImage := range mainImages {
			...
			if sortNum != mainImage.MainOrder {
				input = append(input, image.UpdateSortNumInput{
					ImageID: mainImage.RoomImageDirectID,
					SortNum: sortNum,
				})
			}
		}
	...
	if request.CategoryCd != imageData.CategoryCd {
		// 対象データ取得
		roomImages, err := imageTxRepo.FetchRoomImagesByRoomImageDirectID(request.ImageID)
		if err != nil {
			i.IDirectRepository.TxRollback(tx)
			return err
		}
		if len(roomImages) > 0 {
			// 対象データ削除
			if err := imageTxRepo.ClearRoomImageByRoomImageDirectID(request.ImageID); err != nil {
				i.IDirectRepository.TxRollback(tx)
				return err
			}
			for _, roomImage := range roomImages {
				...
				for index, value := range roomImagesByRoomTypeId {
					...
					if orderNum != value.Order {
						input := &image.HtTmRoomOwnImagesDirects{
	...
	// コミットとロールバック
	if err := i.IDirectRepository.TxCommit(tx); err != nil {
		i.IDirectRepository.TxRollback(tx)
		return err
	}
	return nil
...
func (i *imageDirectUsecase) UpdateIsMain(request *image.UpdateIsMainInput) error {

	// ソート順更新
	if err := i.IDirectRepository.UpdateIsMain(request); err != nil {
		return err
	}
	// 画像情報取得
	imageData, fetchErr := i.IDirectRepository.FetchImageByRoomImageDirectID(request.ImageID)
	if fetchErr != nil {
		return fetchErr
	}
	// メイン画像から外す
	if !request.IsMain {
		// メイン画像取得
		mainImages, fetchImagesErr := i.IDirectRepository.FetchMainImages(imageData.PropertyID)
		if fetchImagesErr != nil {
			return fetchImagesErr
		}
		...
		// 更新するデータがあればソート順更新
		if len(input) > 0 {
			if err := i.IDirectRepository.TrUpdateImageSort(&input); err != nil {
				return err
			}
		}
	}
...
func (i *imageDirectUsecase) UpdateSortNum(request *[]image.UpdateSortNumInput) error {
	return i.IDirectRepository.TrUpdateImageSort(request)
}

// Delete 画像削除
func (i *imageDirectUsecase) Delete(imageID int64) error {
	imageData, fetchErr := i.IDirectRepository.FetchImageByRoomImageDirectID(imageID)
	if fetchErr != nil {
		return fetchErr
	}
	...
	// 対象データ取得
	roomImages, err := imageTxRepo.FetchRoomImagesByRoomImageDirectID(imageID)
	if err != nil {
		return err
	}
	if len(roomImages) > 0 {
		// 対象データ削除
		if err := imageTxRepo.ClearRoomImageByRoomImageDirectID(imageID); err != nil {
			i.IDirectRepository.TxRollback(tx)
			return err
		}
		for _, roomImage := range roomImages {
...
// Create 画像作成
func (i *imageDirectUsecase) Create(request *image.UploadInput, file *multipart.FileHeader, hmUser account.HtTmHotelManager) error {
	// upload情報設定する
	...
	return i.IDirectRepository.CreateImage(upData)
}

// CountMainImages メイン画像の数を数える
func (i *imageDirectUsecase) CountMainImages(propertyID int64) int64 {
	return i.IDirectRepository.CountMainImagesPerPropertyID(propertyID)
}
