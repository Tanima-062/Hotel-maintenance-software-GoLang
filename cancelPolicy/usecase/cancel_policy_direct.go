...
// cancelPolicyDirectUsecase 直仕入れのキャンセルポリシー関連usecase
type cancelPolicyDirectUsecase struct {
	CDirectRepository cancelPolicy.ICancelPolicyDirectRepository
...
// NewCancelPolicyDirectUsecase インスタンス生成
func NewCancelPolicyDirectUsecase(db *gorm.DB) cancelPolicy.ICancelPolicyUsecase {
	return &cancelPolicyDirectUsecase{
		CDirectRepository: infra.NewCancelPolicyDirectRepository(db),
...
// Detail キャンセルポリシー詳細
func (c *cancelPolicyDirectUsecase) Detail(req *cancelPolicy.DetailInput) (*cancelPolicy.CancelPolicyJSONWithName, error) {
	response := &cancelPolicy.CancelPolicyJSONWithName{}
	// ht_tm_property_directsにデータが存在しないこともある為
	if req.PropertyID != nil {
		facilityData, err := c.FDirectRepository.FirstOrCreate(*req.PropertyID)
		if err != nil {
			return response, err
		}
...
// Save キャンセルポリシー保存
func (c *cancelPolicyDirectUsecase) Save(req *cancelPolicy.UpdateInput) error {
	...
	if req.PlanCancelPolicyID != nil {
		...
		err = c.CDirectRepository.Update(*req.PropertyID, string(jsonData))
	} else {
...
func (c *cancelPolicyDirectUsecase) Create(req *cancelPolicy.CreateInput) error {
	cancelPolicy := req.CancelPolicyJSON
	jsonData, jErr := json.Marshal(cancelPolicy)
	if jErr != nil {
		return jErr
	...
	return c.CCommonRepository.CreatePlanCancelPolicy(req.CancelPolicyName, utils.WholesalerIDDirect, req.PropertyID, string(jsonData))
}

// List プランごとのキャンセルポリシー一覧返却
func (c *cancelPolicyDirectUsecase) List(req *cancelPolicy.ListInput) ([]cancelPolicy.CancelPolicyInfo, error) {
...
func (c *cancelPolicyDirectUsecase) Delete(req *cancelPolicy.DeleteInput) error {
	return c.CCommonRepository.DeletePlanCancelPolicy(req.PlanCancelPolicyID)
}

// PlanList プラン一覧返却
func (c *cancelPolicyDirectUsecase) PlanList(req *cancelPolicy.PlanListInput) ([]cancelPolicy.PlanInfo, error) {
	list, err := c.PDirectRepository.FetchAllByCancelPolicyID(req.PlanCancelPolicyID)
	if err != nil {
