...
// cancelPolicyNeppanUsecase ねっぱんのキャンセルポリシー関連usecase
type cancelPolicyNeppanUsecase struct {
	CNeppanRepository cancelPolicy.ICancelPolicyNeppanRepository
...
// NewCancelPolicyNeppanUsecase インスタンス生成
func NewCancelPolicyNeppanUsecase(db *gorm.DB) cancelPolicy.ICancelPolicyUsecase {
	return &cancelPolicyNeppanUsecase{
		CNeppanRepository: infra.NewCancelPolicyNeppanRepository(db),
...
// Detail キャンセルポリシー詳細
func (c *cancelPolicyNeppanUsecase) Detail(req *cancelPolicy.DetailInput) (*cancelPolicy.CancelPolicyJSONWithName, error) {
	response := &cancelPolicy.CancelPolicyJSONWithName{}
	// ht_tm_property_directsにデータが存在しないこともある為
	if req.PropertyID != nil {
		facilityData, err := c.FNeppanRepository.FirstOrCreate(*req.PropertyID)
		if err != nil {
			return response, err
		}
...
// Save キャンセルポリシー保存
func (c *cancelPolicyNeppanUsecase) Save(req *cancelPolicy.UpdateInput) error {
	...
	if req.PlanCancelPolicyID != nil {
		...
		err = c.CNeppanRepository.Update(*req.PropertyID, string(jsonData))
	} else {
...
func (c *cancelPolicyNeppanUsecase) Create(req *cancelPolicy.CreateInput) error {
	cancelPolicy := req.CancelPolicyJSON
	jsonData, jErr := json.Marshal(cancelPolicy)
	if jErr != nil {
		return jErr
	...
	return c.CCommonRepository.CreatePlanCancelPolicy(req.CancelPolicyName, utils.WholesalerIDNeppan, req.PropertyID, string(jsonData))
}

// List キャンセルポリシー一覧返却
func (c *cancelPolicyNeppanUsecase) List(req *cancelPolicy.ListInput) ([]cancelPolicy.CancelPolicyInfo, error) {
...
// Delete プランごとのキャンセルポリシー一覧返却
func (c *cancelPolicyNeppanUsecase) Delete(req *cancelPolicy.DeleteInput) error {
	return c.CCommonRepository.DeletePlanCancelPolicy(req.PlanCancelPolicyID)
}

// PlanList プラン一覧返却
func (c *cancelPolicyNeppanUsecase) PlanList(req *cancelPolicy.PlanListInput) ([]cancelPolicy.PlanInfo, error) {
	list, err := c.PNeppanRepository.FetchAllByCancelPolicyID(req.PlanCancelPolicyID)
	if err != nil {
